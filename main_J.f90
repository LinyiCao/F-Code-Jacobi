program main
    use mkl95_lapack            !for calling geev-line 89 to solve the eigen vector of PP
    implicit none
	integer(4)	                :: N 
	...
	real(8), allocatable		:: theta(:), xc(:), xf(:), xp(:), dx(:)
    ...
    do  ! 
        ...
        do irow = 1, N
            ...
            do   ! solve the irow-th equation
                ...
                call functionF(irow, xf, grid, PP, qx, qdx, N, A, alpha, beta, mu, b, &
                     kappa, delta, xcr, fx, fdx)
                ! update the current value
                xf(irow) = xf(irow) - fx(irow) / fdx(irow)
                ! convergence creteria
                if(abs(fx(irow)/fdx(irow)) < tol .or. abs(fx(irow)) < tol) exit
            end do
            xc(irow) = xf(irow) !cause we call functionF with xf
        end do
